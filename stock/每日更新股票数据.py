...
for i in df.index:
    ...
    # 构建存储文件路径
    path = '/Users/fangjiaqi/Desktop/xbx—stock/crawl_stock_data/data/股票数据库/basic-trading-data/stock_data/' \
           + stock_code + '.csv'
    # 文件存在，不是新股
    ...
    # 文件不存在，说明是新股
        ...
        # 先将头文件输出
        pd.DataFrame(columns=['数据由哈库拉马塔塔整理']).to_csv(path, index=False, encoding='gbk')
