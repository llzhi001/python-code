...
def find_cards_info():
    ...
    cards = driver.find_elements_by_css_selector(cards_sel)  #->list ->[]
    info_list = []

    for card in cards:
        content_sel = 'div.WB.text.W_f14'
        ...
        link = card.find_element_by_css_selector(link_sel).get_attribute('herf')
...
def find_next():
    ...
    if next_page:
        return next_page[0].get_attribute('herf')   #url
...
driver = start_chrome()
input()
run_crawler(base, '2017-07-02~2018-03-05')
